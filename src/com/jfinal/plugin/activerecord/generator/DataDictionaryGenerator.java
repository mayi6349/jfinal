		...
		try {
			...
			for (TableMeta tableMeta : tableMetas) {
				...
				while (rs.next()) {
					...
					columnMeta.isNullable = rs.getString("IS_NULLABLE"); // 是否允许
																			// NULL
																			// 值
					...
					if (StringUtils.isEmpty(tableMeta.remarks)) {
						...
						if (tableName.contains("ID") || tableName.contains("Id")) {
							tableMeta.remarks = tableName.replace("ID", "表").replace("Id", "表");
						}
					}
