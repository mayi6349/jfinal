...
public class Db {
	 ...
	 */
	public static List<Record> paginateList(int pageNumber, int pageSize, String sql, Object... paras) {
		sql = sql.toLowerCase();
		int start = sql.indexOf("from");
		String select = sql.substring(0, start);
		String sqlExceptSelect = sql.substring(start,sql.length());
		return dbPro.paginateList(pageNumber, pageSize, select, sqlExceptSelect, paras);
	}
